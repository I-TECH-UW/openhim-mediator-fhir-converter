    ...
    allCases.forEach(subCase => {
        it (`should output a valid FHIR data with ${subCase.dataFile} and ${subCase.templateFile}`, done => {
            ...
            const meta = {
                ...
                '.hl7': [ '/api/convert/hl7v2', constants.HL7V2_TEMPLATE_LOCATION, constants.HL7V2_DATA_LOCATION, 'hl7v2' ]
            };
            ...
            const groundTruthLocation = path.join(__dirname, `./data/${meta[dataType][3]}`, subCase.templateFile);
            ...
            const groundTruthFilePath = path.join(groundTruthLocation, utils.getGroundTruthFileName(subCase));
            const payload = {
            ...
            supertest(app).post(endpointURL).set(API_KEY_HEADER, API_KEY).send(payload)
                .expect(200)
                .expect(response => {
